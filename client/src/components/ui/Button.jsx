...
const Button = forwardRef(({
  ...
  const hasLeftIcon = !isLoading && LeftIcon;
  const hasRightIcon = !isLoading && RightIcon;
  const hasGap = (hasLeftIcon || hasRightIcon) && children;

  // Filter out any unwanted props that might cause rendering issues
  const filteredProps = {};
  Object.keys(props).forEach(key => {
    if (key !== 'disabled' && key !== 'className' && key !== 'children') {
  ...
  return (
    <button
      ref={ref}
      type={type}
      className={`${baseClasses} ${hasGap ? 'gap-2' : ''}`}
